...
export default function QuizPage() {
  ...
  // Function to poll current question (only when needed)
  ...
  // When quiz is LIVE, use WebSocket events primarily, with minimal polling fallback
  useEffect(() => {
    if (quizState === 'LIVE') {
      // Join the quiz first
      ...
      // Initial poll to get current question
      pollCurrentQuestion();
      
      // Fallback poll every 60 seconds (only if WebSocket fails)
      // WebSocket events should handle most updates
      questionIntervalRef.current = setInterval(() => {
        // Only poll if we don't have a current question or if timer expired
        if (!currentQuestion || (expiresAtRef.current && Date.now() >= expiresAtRef.current)) {
          pollCurrentQuestion();
        }
      }, 60000);
